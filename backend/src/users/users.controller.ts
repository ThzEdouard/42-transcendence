...
export class UsersController {
	...
	async handleUpload(@UploadedFile(
		...
		fs.unlink('.' + user.avatarPath.slice(user.avatarPath.indexOf('/users/') + 6), (err) => {
			if (err) {
				console.error(err);
				return ;
			}
		...
		await this.usersService.update(parseInt(id), { avatarPath: `http://${process.env.HOSTNAME}:${process.env.PORT}/api/users/imgs/` + file.filename });
		return {
			statusCode: 200,
			data: `http://${process.env.HOSTNAME}:${process.env.PORT}/api/users/imgs/` + file.filename,
		}
	}
